...
- `worktree set --parent-worktree <selector>` **reparents an existing worktree**; `--no-parent` on `set` detaches it to top level. Use it when the owner of a child changes — a coordinator handing its run to a successor, or work adopted by another workspace — so the child does not stay under a dead parent. `set` takes worktree selectors only, not the `folder:` parent context keys `create` accepts.
